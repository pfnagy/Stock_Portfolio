...
def add_stock(stock_portfolio):
    ticker_symbol = input("Enter a ticker symbol to add")
    qty = int(input("How many did you buy?"))
    stock_portfolio[ticker].append()
    #fix this---changed how portfolio is
...
def portfolio(stock_portfolio):
    # to hold portfolio
    stock_portfolio = [[],[],[],[],[]]
    ticker=stock_portfolio(0)
    company=stock_portfolio(1)
    qty=stock_portfolio(2)
    price=stock_portfolio(3)
    value=stock_portfolio(4)
